  ...
  return (
    <footer className="fixed inset-x-0 bottom-0 z-20 border-t bg-background/90 backdrop-blur">
      <div
        ...
        title="Click to jump"
      >
        <div className="h-full bg-indigo-400/90 transition-[width] duration-300" style={{ width: `${pct}%` }} />
      </div>
        ...
        <div className="flex items-center">
          <Select
            ...
            onValueChange={async (v) => {
              if (!(await player.setVoice(v))) toast.error("Voice change failed — is the server running?")
            }}
          >
            <SelectTrigger size="sm" className="w-44 max-sm:w-24" title="Narrator voice">
