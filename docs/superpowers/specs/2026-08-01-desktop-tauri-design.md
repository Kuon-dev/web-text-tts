...
| macOS floor | `minimumSystemVersion: "13.3"`, Vite `target: safari16`. | `watermark.ts:81` uses regex lookbehind with **no fallback** — a SyntaxError at module parse would take the whole bundle down. That is WebKit 16.4. `oklch()`/`color-mix(in oklab)` need 16.2+. |
